...
import { REGISTRATION_OPEN, FUNDRAISING_CLOSE, nextRideEvent } from '../../types/constants';
...
export function SummaryCards({ survivors, highRollers, avgDonation }: SummaryCardsProps) {
  ...
  const event = nextRideEvent(now);
  const daysToRide = Math.max(0, Math.ceil((event.date.getTime() - now.getTime()) / 86400000));
  ...
  const cards = [
    ...
    { value: daysToRide > 0 ? String(daysToRide) : event.dayText, label: `Days to ${event.label}` },
